...
uint8_t message2[256] __attribute__((coherent, aligned(4)));
    ...
    switch(appData.state)
    {
        ...
        case APP_STATE_CHECK_CDC_READ:
            ...
            if(appData.isReadComplete == true)
            {
                ...
                sscanf(appData.readBuffer,"%s", message2);
                int n;
                for(n=0;n<256;n++){
                    if(message2[n]=='a') {
                    ...
                    if(message2[n]=='P'){
                        COM1 = ((message2[n+2]<<8)|(message2[n+1]&0xff))-320;

                        RDIR = RFOR;
                        LDIR = LFOR;
                        //if((COM1)>0){
                            LPWM = MAXSPEED+(COM1*Kp);
                            RPWM = MAXSPEED-(COM1*Kp);
                       // }
                       // else {
                      //      LPWM = MAXSPEED-(COM1*0.125);
                       //     RPWM = MAXSPEED+(COM1*0.25);
                      //  }
                        if(LPWM>100)LPWM=100;
                        if (RPWM>100) RPWM = 100;
                        OC1RS = RPWM*(PR2+1)/100;
                        OC3RS = LPWM*(PR2+1)/100;
                    }
